...
A Java movie and actor lookup application with rating and collaboration queries.

## Technical depth

File ingestion, entity relationships, normalization, and query logic.
...
Keep moviedata.txt and movieratings.csv in the working directory. MovieTrivia and file.MovieDB provide the main operations.
...
The repackaged source compiled with JDK 24. The local JUnit run passed **14 tests**, with 0 failures. Maven configuration is provided for convenience; the reported run used javac and JUnit Console directly. Passing tests do not establish exhaustive correctness or production readiness.
